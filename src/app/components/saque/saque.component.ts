...
export class SaqueComponent {
  ...
  constructor(
    public dialogRef: MatDialogRef<SaqueComponent>,
  ...
  realizarSaque(): void {
    if (!this.contaSelecionada || this.valorSaque <= 0) {
      this.snackBar.open('Selecione uma conta e informe um valor válido', 'Fechar', { duration: 3000 });
      return;
    }

    if (this.valorSaque > (this.contaSelecionada.saldo + this.contaSelecionada.limite)) {
      this.snackBar.open('Saldo insuficiente', 'Fechar', { duration: 3000 });
      return;
    }

    this.contaService.realizarSaque(this.contaSelecionada.id, this.valorSaque).subscribe({
      next: () => {
